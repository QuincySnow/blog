...
description: Firefox 衍生品浏览器的性能调优与隐私技术应用
...
**技术背景**：不同浏览器对视频编码的支持情况不同，网站可以通过检测浏览器支持的编码格式来识别浏览器类型和版本，这可作为指纹识别的一部分。

**工具**：[h264ify](https://addons.mozilla.org/firefox/addon/h264ify/)

**配置方案**：

| 选项              | 推荐设置 | 说明                              |
| ----------------- | -------- | --------------------------------- |
| Block 60fps video | 可选     | 屏蔽 60fps 视频支持，限制为 30fps |
| Block h264 (AVC)  | 未选     | 保留 H.264 支持（兼容性最好）     |
| Block VP8         | ✓ 选中   | 屏蔽 VP8 编码格式支持             |
| Block VP9         | ✓ 选中   | 屏蔽 VP9 编码格式支持             |
| Block AV1         | 未选     | 保留 AV1 支持（现代格式）         |

**音频格式**：

| 选项                           | 推荐设置 | 说明                               |
| ------------------------------ | -------- | ---------------------------------- |
| Block Opus                     | 未选     | 保留 Opus 音频支持                 |
| Block AAC (MP4A)               | 未选     | 保留 AAC 支持（兼容性好）          |
| Disable Loudness Normalization | 可选     | 禁用响度标准化可进一步减少指纹特征 |

**原理**：通过限制浏览器声称支持的编码格式，使浏览器在媒体协议协商时呈现更"标准化"的能力集，减少独特的编码支持组合成为识别特征。

**权衡**：某些网站可能默认使用 VP9/VP8 而不支持 H.264 回退，可能导致视频无法正常播放。建议在出现问题时针对特定网站白名单。

**验证方法**：访问 [Media Source Extensions 测试页面](https://www.youtube.com/html5)，查看浏览器声称支持的编码格式列表。
...
- [ ] 安装 uMatrix，配置跨域请求阻止和存储限制
- [ ] 安装 h264ify，禁用 VP8/VP9 视频编码标准化媒体格式
