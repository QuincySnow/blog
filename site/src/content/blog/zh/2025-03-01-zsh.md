...
Zsh 本身不带插件管理，社区的常见选择有四个：

| 框架      | 特点                                                   | 启动速度             | 适合谁                             |
| --------- | ------------------------------------------------------ | -------------------- | ---------------------------------- |
| Oh My Zsh | 生态最大，开箱即用，几千个插件和主题                   | 慢（常见 0.5s~2s+）  | 新手、想要最多少配置的人           |
| **Zinit** | 按需加载（turbo mode）、支持异步、无侵入、纯 Zsh       | **最快（可 <50ms）** | 追求启动速度、想要精细控制的人     |
| Antidote  | 静止的插件管理器（zsh 官方生态），速度快、配置简单     | 快                   | 想要简单 + 速度折中的 Oh My Zsh 用户 |
| Prezto    | 老牌框架，配置集中，更新较慢                           | 中等                 | 已用多年不愿迁移的人               |

**推荐 Zinit**：它比 Oh My Zsh 快一个量级，而且不需要“抛弃”已有配置——插件可以逐个迁移，不必一次性重写 `.zshrc`。
...
| 命令                       | 作用                                     |
| -------------------------- | ---------------------------------------- |
| `zinit light <插件>`       | 加载插件（不变更仓库状态，最快）         |
| `zinit load <插件>`        | 加载并跟踪更新（会 `git fetch`）         |
| `zinit ice <选项>`         | 为下一条 `light/load` 设置参数（如 `wait`） |
| `zinit snippet <片段>`     | 加载单个脚本，如 `OMZ::plugins/xxx`      |
| `zinit update --all`       | 更新所有插件                             |
| `zinit delete <插件>`      | 删除插件                                 |
| `zinit list`               | 列出已加载插件                           |

**Turbo 加载（Zinit 的核心优势）**：`wait` 让插件在提示符出现之后再异步加载，启动耗时几乎为零：
...
Antidote 是 Zsh 官方生态里推荐的**静态**插件管理器——启动时一次性生成静态加载脚本，比 Oh My Zsh 快很多：
...
Prezto 把插件叫**模块**，内置模块无需下载，改 `~/.zpreztorc` 的 `zstyle ':prezto:load' pmodule ...` 列表即可：
...
第三方插件和 Oh My Zsh 一样手动 clone，但目录在 `~/.zprezto/modules/`（或 `contrib`）：

```bash
git clone https://github.com/zsh-users/zsh-autosuggestions \
  ~/.zprezto/modules/autosuggestions
```
