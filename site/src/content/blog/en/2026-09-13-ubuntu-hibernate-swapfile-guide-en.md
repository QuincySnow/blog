...
Ubuntu 26.04 generates initramfs with **dracut** (not the older initramfs-tools). dracut doesn't necessarily include resume support by default, so declare two things explicitly:
...
If you're configuring manually like me, add the button back with an extension. The one that works on GNOME 50:
...
cd hibernate-status-gnome50 && ./install.sh  # adds Hibernate to the power menu
```
...
└── Hibernate
    ...
    ├── GNOME 50: hibernate-power-menu extension
...
7. GNOME 50 has no Hibernate button by default — the `hibernate-power-menu@arnakazim` extension adds it.
